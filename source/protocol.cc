...
*
*              TODO: This file is now hardcoded for microphone on channel 1
*              at 16 kHz and to some degree prepared for IMU data on channel 2.
*              It should be made generic so the main function can register any
*              sensors and so the device responds accordingly.
*
* Related Document: See PROTOCOL.md
*
*******************************************************************************
* Copyright 2023, Imagimob AB (an Infineon company) or an affiliate of
...
void protocol_repl()
{
	...
	bytes_read = streaming_receive(receive_p, RECEIVE_BUFFER_SIZE - (receive_p - receive_buffer));
    if (bytes_read)
    {
